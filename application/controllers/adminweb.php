...
class adminweb extends CI_Controller {
	...
	public function seo_simpan() {
		...
		$this->admin_model->UpdateSeo($id_seo,$tittle,$keyword,$description);redirect("adminweb/seo");
	}
	//Akhir Seo
   ...
   //Awal kontak
	...
	public function kontak_simpan() {
		...
		$this->admin_model->Simpankontak($id_kontak,$alamat,$phone,$email);redirect("adminweb/kontak");
	}
	//Akhir kontak
   ...
   public function sosial_media_simpan() {
		...
		$this->admin_model->SimpanSosialMedia($id_sosial_media,$tw,$fb,$gp);redirect("adminweb/sosial_media");
	}
   //Akhir Sosial Media
	...
	public function tentangkami_simpan() {
		...
		$this->admin_model->UpdateTentangkami($id_tentangkami,$judul,$deskripsi);redirect("adminweb/tentangkami");
	}
	//Akhir Tentang Kami
	...
	public function carabelanja_simpan() {
		...
		$this->admin_model->UpdateCarabelanja($id_carabelanja,$judul,$deskripsi);redirect("adminweb/carabelanja");
	}
	//Akhir Cara Belanja
	...
	public function admin_simpan() {
			...
			if ($this->form_validation->run() == FALSE)
			{
				$this->template->load('template','adminweb/admin/add');
						...
						$this->db->insert("tbl_user",$in_data);
	...
	public function admin_edit() {
		...
		foreach ($query->result_array() as $tampil) {
			...
			$data['nama_user'] = $tampil['nama_user'];
			$data['email'] = $tampil['email'];
	...
	public function buku_tamu_balas_simpan() {
		...
		$this->email->from('irvanperssi12@gmail.com', 'IMC Online Shop');
		$this->email->to($email); 	
	...
	public function nota(){
		if($this->session->userdata("logged_in")!=="") {
			$this->template->load('template', 'adminweb/nota/nota_v');
		}else{
			redirect("adminweb");
		}
	}
