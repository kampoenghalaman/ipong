...
class pegawai_model extends CI_Model {
	...
	function CekAdminLogin($data) {
		...
		$cek['password'] = md5($data['password']);
	...
	//Awal Pegawai service
	function Getpegser() {
		return $this->db->query("select *from tbl_peg_service order by id_peg_service desc");
	}

	function DeletePeg($id) {
		return $this->db->query("delete from tbl_peg_service where id_peg_service='$id'");
	}

	function GetPegEdit($id) {
		return $this->db->query("select * from tbl_peg_service where id_peg_service='$id' ");
	}
